...
enum ExpanduinoCommandMeta {
  ...
  EXPANDUINO_CMD_META_DEVICE_NAME                     = 4,
  EXPANDUINO_CMD_META_DEVICE_PHYS                     = 5,
  EXPANDUINO_CMD_META_VENDOR_NAME                     = 6,
  EXPANDUINO_CMD_META_PRODUCT_NAME                    = 7,
  EXPANDUINO_CMD_META_SERIAL_NUMBER                   = 8,
  EXPANDUINO_CMD_META_PHYS                            = 9
};
...
class ExpanduinoSubdevice {
  ...
  uint8_t devNum;
  const char* name;
  const char* physPrefix;
  
  friend class Expanduino;
  friend class MetaExpanduinoSubdevice;
  
public:
  ExpanduinoSubdevice(Expanduino& container, ExpanduinoSubdeviceType devType, const char* name, const char* physPrefix);
  ...
  void getPhysicalLocation(Print& out);
};
...
public:
  ...
  virtual void getVendorName(Print& out);
  virtual void getProductName(Print& out);
  virtual void getSerialNumber(Print& out);
  virtual void getPhysicalLocation(Print& out) = 0;
};